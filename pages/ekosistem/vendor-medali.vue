...
// --- SEO ---
const pageTitle = 'Daftar Vendor Pembuat Medali Custom'
const pageDescription =
  'Lagi cari vendor medali custom untuk event lari kamu? Berikut Daftar Pembuat Medali Custom di Indonesia yang bisa kamu kontak!'
useSeoMetaDynamic({
  title: pageTitle,
  description: pageDescription,
  url: '/ekosistem/vendor-medali',
})
...
async function fetchItems(page = 1, append = false) {
  ...
  try {
    const response = await getVendors({ type: 'medali', page, per_page: 12 })
    if (append) {
    ...
    itemListElement: computed(() =>
      items.value.map((vendor: Vendor, index: number) => ({
        ...
        item: {
          '@type': 'Organization',
          name: vendor.name,
          url: `${config.public.siteUrl}/ekosistem/vendor-medali#${vendor.id}`,
        },
      }))
    ),
  }),
...
const breadcrumbItems = computed<BreadcrumbItem[]>(() => [
  ...
  { text: 'Vendor Medali', link: null },
])
const headerBg = computed(() => getImage('header_bg_vendor_medali', null) ?? undefined)
const { data: vendorMedaliHeaderBanners } = await useAsyncData(
  'ad-banners-page-header-vendor-medali',
  () => fetchResponsiveBanners('page_header_vendor_medali')
)
const headerAdBanners = computed(() => vendorMedaliHeaderBanners.value?.desktop ?? [])
const headerAdBannersMobile = computed(() => vendorMedaliHeaderBanners.value?.mobile ?? [])

// --- SEO Injection ---
const pillarKeyword = SEO_KEYWORDS.vendorMedaliLari
</script>
      ...
      <div class="layout-container">
        ...
        <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p class="text-sm text-gray-600">
              Total <span class="font-semibold text-primary">{{ totalCount }}</span> vendor
              terdaftar.
            </p>
          </div>
          <div
        ...
        >
          ...
          <p class="mt-2 text-gray-500">
            Data vendor medali belum tersedia. Silakan cek kembali nanti.
          </p>
        </div>
      </div>
    </section>
